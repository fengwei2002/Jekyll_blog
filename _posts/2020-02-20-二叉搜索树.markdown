...
AVL中适度平衡所用到的指标 ：对于任意一个节点 v 左子树与右子树的高度差
...
同时可有多个失衡点；最低者不低于x祖父; 
g经过单旋调整后复衡；子树高度复原；更高的祖先也必然平衡；全树平衡

zagzag zigzig zig为顺时针旋转；zag为逆时针旋转
...
template<typename T> BinaryNodePosition(T) AVL<T>::insert( const T & e ){
    ...
    for ( BinaryNodePosition(T) g = x->parent; g; g = g->parent ) {
        if ( !AvlBalanced( *g ) ) {  //一旦发现失衡，则通过调整回复平衡
...
![230f4e0034aa0ef78a64bcc3e819f2d.jpg](https://raw.githubusercontent.com/fengwei2002/picgotest/master/img/230f4e0034aa0ef78a64bcc3e819f2d.jpg)
